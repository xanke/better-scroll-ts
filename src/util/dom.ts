...
// 返回浏览器内核识别码 -ms IE, -moz 火狐, -webkit 谷歌, -o opera, standard 标准
...
export function removeEvent(el, type, fn, capture) {
  el.removeEventLister(type, fn, { passive: false, capture: !!capture })
}
...
export function offsetToBody(el) {
  ...
  // window.pageXOffset, window.pageYOffset 返回文档在窗口滚动距离
  return {
...
export const hasTransition =
  inBrowser && prefixStyle('transition') in elementStyle
